...
public class NestedType {
    ...
    public boolean tryFillFromExpr(SQLExpr expr) {
        ...
        // inner_hits
        if (size == 3) {
            this.innerHits = ParseUtils.extendedToString(parameters.remove(--size));
        }

        String field = ParseUtils.extendedToString(parameters.get(0));
        this.field = field;
        if (size == 1) {
            // calc path myself..
            if (!field.contains(".")) {
            ...
            if (secondParameter instanceof SQLTextLiteralExpr || secondParameter instanceof SQLIdentifierExpr || secondParameter instanceof SQLPropertyExpr) {
                ...
                String pathString = ParseUtils.extendedToString(secondParameter);
